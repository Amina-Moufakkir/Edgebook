...
- **Session revocation timing — resolved 2026-08-01 by amending ADR 0004.** Its recovery bullet now states "on password change, not on reset request" directly. Retained for the record, because it is a documentation lesson rather than a decision: 0004 always specified the safe reading, but split across two subsections — *Account recovery* said only "recovery invalidates existing sessions," while *Session handling* carried the timing. Read together they were correct; read alone the recovery bullet appeared to permit revocation on request, which would be a denial-of-service vector. A provision that is only correct when read beside another provision is a provision waiting to be misread.
- **Recovery and MFA — resolved 2026-08-01 by amending ADR 0004.** This flow surfaced a genuine gap in an Accepted record: 0004 required MFA for administrative access but said nothing about whether recovery challenges it, which made email access alone sufficient for takeover. 0004 now requires recovery to satisfy every enrolled factor, and this flow's Primary Path is bound by it — where MFA is enrolled, the second factor is challenged before a password may be changed. **What remains open is the consequence:** what happens when the second factor is also lost (recovery codes, support-mediated verification, or no path at all). That is an open decision in 0004's Amendments section and must be answered before SF-03 is drafted.
